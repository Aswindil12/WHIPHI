...
4. Download and open [WiPHi](https://github.com/Aswindil12/WHIPHI/blob/main/ESP8266_WiPHi.ino) with Arduino IDE
6. Select an `ESP8266 Deauther` board in Arduino under `tools` -> `board`
7. Connect your device and select the serial port in Arduino under `tools` -> `port`
8. Click Upload button

# How to use:
- Connect to the AP named `WIPHI` with password `00000000` from your phone/PC.
...
- Once a correct password is found, AP will be restarted with default ssid `WIPHI` / `00000000` and at the bottom of a table you should be able to see something like "Successfully got password for - `TARGET_SSID` - `PASSWORD`
